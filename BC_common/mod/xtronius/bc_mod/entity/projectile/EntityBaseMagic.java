...
import mod.xtronius.bc_mod.util.BCDamageSource;
...
public class EntityBaseMagic extends Entity implements IProjectile
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityBaseMagic(World par1World)
    {
        super(par1World);
    ...
    public EntityBaseMagic(World par1World, double par2, double par4, double par6)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityBaseMagic(World par1World, EntityLivingBase par2EntityLivingBase, EntityLivingBase par3EntityLivingBase, float par4, float par5)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityBaseMagic(World par1World, EntityLivingBase par2EntityLivingBase, float par3)
    {
        super(par1World);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = par2EntityLivingBase;
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            {
            	for (int buff = 0; buff < 12; ++buff)
                {
            		this.worldObj.playSoundAtEntity(this, "liquid.splash", 0.5F, 0.4F / (rand.nextFloat() * 0.4F + 0.8F));
        ...
        else
        {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null)
                {
                    ...
                    {
                        damagesource = BCDamageSource.causeWaterBoltDamage(this, this);
                        //this.worldObj.playSoundAtEntity(this, "liquid.splash", 0.5F, 0.4F / (rand.nextFloat() * 0.4F + 0.8F));
                        for (int buff = 0; buff < 12; ++buff)
                        {
                            this.worldObj.spawnParticle("splash", this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D);
                    ...
                    {
                        damagesource = BCDamageSource.causeWaterBoltDamage(this, this.shootingEntity);
                        
                        for (int buff = 0; buff < 8; ++buff)
                        {
                            this.worldObj.spawnParticle("splash", this.posX, this.posY, this.posZ, 0.0D, 0.0D, 0.0D);
                        }
                    }
                ...
                for (l = 0; l < 5; ++l)
                {
                    this.worldObj.spawnParticle("splash", this.posX + this.motionX * (double)l / 4.0D, this.posY + this.motionY * (double)l / 4.0D, this.posZ + this.motionZ * (double)l / 4.0D, -this.motionX, -this.motionY + 0.2D, -this.motionZ);
                }
